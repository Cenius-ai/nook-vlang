...
<div class="max-w-4xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        ...
        <span class="px-3 py-1 text-sm rounded-full bg-[#c9302d]/10 text-[#c9302d] font-medium">
            {{ $flags->where('status', 'pending')->count() }} pending
        </span>
    </div>

    @forelse($flags as $flag)
        <div class="mb-3 bg-[#fdffff] dark:bg-[#132129] rounded-lg border {{ $flag->status === 'pending' ? 'border-[#c9302d]/40 dark:border-[#c9302d]/40' : 'border-[#d9dfe2] dark:border-[#2a353a]' }} p-5">
            <div class="flex items-start justify-between gap-4">
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 mb-2">
                        <span class="px-2 py-0.5 text-xs rounded-full font-medium {{ $flag->status === 'pending' ? 'bg-[#c9302d]/10 text-[#c9302d]' : 'bg-[#e8eef1] dark:bg-[#1a282f] text-[#5a656b] dark:text-[#8a959b]' }}">
                            {{ ucfirst($flag->status) }}
                        </span>
                        <span class="text-xs text-[#5a656b] dark:text-[#8a959b]">
                            Flagged by <a href="{{ route('users.profile', $flag->user) }}" class="text-[#009494] hover:underline">{{ $flag->user->name }}</a>
                            {{ $flag->created_at->diffForHumans() }}
                        </span>
                    </div>

                    <p class="text-sm text-[#0b1418] dark:text-[#e8eef1] mb-2"><strong>Reason:</strong> {{ $flag->reason }}</p>

                    {{-- Show flagged content --}}
                    @php $content = $flag->flaggable; @endphp
                    @if($content)
                        <div class="p-3 rounded-lg bg-[#e8eef1] dark:bg-[#1a282f] text-sm text-[#5a656b] dark:text-[#8a959b] mb-2">
                            <span class="text-xs font-medium uppercase tracking-wide text-[#5a656b] mb-1 block">
                                ...
                                @if($content instanceof \App\Models\Answer)
                                    on "<a href="{{ route('questions.show', $content->question) }}" class="text-[#009494] hover:underline">{{ \Illuminate\Support\Str::limit($content->question->title, 60) }}</a>"
                                @endif
                            </span>
                            <p class="whitespace-pre-wrap break-words">{{ \Illuminate\Support\Str::limit($content instanceof \App\Models\Question ? $content->title : $content->body, 200) }}</p>
                        </div>
                    @else
                    ...
                    @if($flag->resolver)
                        <p class="text-xs text-[#5a656b]">
                            Resolved by {{ $flag->resolver->name }} {{ $flag->resolved_at?->diffForHumans() }}
                        </p>
                    @endif
                </div>
                ...
                @if($flag->status === 'pending')
                    <div class="flex flex-col gap-2 shrink-0">
                        <button onclick="resolveFlag({{ $flag->id }}, 'dismiss')"
                        ...
                        <button onclick="resolveFlag({{ $flag->id }}, 'delete')"
