...
<div class="flex flex-col lg:flex-row gap-8">
    <div class="flex-1 min-w-0">
        <div class="flex items-center gap-3 mb-6">
            <span class="px-3 py-1.5 text-sm rounded-lg bg-[#009494] text-white font-medium">{{ $tag->name }}</span>
            <span class="text-sm text-[#5a656b] dark:text-[#8a959b]">{{ $tag->questions_count }} {{ Str::plural('question', $tag->questions_count) }}</span>
        </div>
            ...
            <div class="mb-3 bg-[#fdffff] dark:bg-[#132129] rounded-lg border border-[#d9dfe2] dark:border-[#2a353a] p-5">
                <div class="flex gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-sm text-[#5a656b] dark:text-[#8a959b] line-clamp-2 mb-2">{{ \Illuminate\Support\Str::limit(strip_tags($question->body), 160) }}</p>
                        <span class="text-xs text-[#5a656b]">asked {{ $question->created_at->diffForHumans() }} by <a href="{{ route('users.profile', $question->user) }}" class="text-[#009494] hover:underline">{{ $question->user->name }}</a></span>
                    </div>
                </div>
            </div>
        @empty
